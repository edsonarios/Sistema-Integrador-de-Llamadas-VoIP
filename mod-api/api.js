...
api.post('/getUsuariosPorSala', async (req, res, next) => {
  var params = req.body
  //obtenemos todos los usuarios y sus numeros sips
...
api.post('/deleteSala', async (req, res, next) => {
  const params = req.body
  //obetenemos todos los sips e iaxs de los usuarios
  const sipsAll = await Sip.findAll()
  const iaxsAll = await Iax.findAll()
  //creamos vectores vacios y una variable
  let sipsdeusuario = []
  let iaxsdeusuario = []
  let edit
  //iteramos sobre los sips
  sipsAll.forEach((obj) => {
    //preguntamos si el contexto es igual al contexto que enviamos por postman
    if (obj.context == params.context) {
      //guardamos todo el objeto en el vector 1
      sipsdeusuario.push(obj)
    }
  })
  //iteramos sobre los iaxs
  iaxsAll.forEach((obj) => {
    //preguntamos si el contexto es igual al contexto que enviamos por postaman
    if (obj.context == params.context) {
      //guardamos todo el objeto en el vector 2
      iaxsdeusuario.push(obj)
    }
  })
  //iteramos sobre el vector 1
  sipsdeusuario.forEach((obj) => {
    //editamos su contexto por el nuevo contexto llamado nuevaSala
    try {
...
api.post('/getUsuariosWithSipsAndIaxs', async (req, res, next) => {
  ...
  const sipsAll = await Sip.findAll()
  const iaxsAll = await Iax.findAll()
  //Creo vectores vacios
  let getsips = []
  let getiaxs = []
  let todos = []
  //itero sobre los sips
  sipsAll.forEach((obj) => {
    //preguntamos si el usuarioId es igual al id que mandamos por postman
    if (obj.usuarioId == params.id) {
      //guardamos todo el objeto en el vector
      getsips.push(obj)
    }
  })
  //itero sobre los iaxs
  iaxsAll.forEach((obj) => {
    //preguntamos si el usuarioId es igual al id que mandamos por postman
    if (obj.usuarioId == params.id) {
      //guardamos todo el objeto en el vector
...
api.post('/ListarHistorialByFechaBySipsAndIaxs', async (req, res, next) => {
  ...
  const sipsAll = await Sip.findAll()
  //declaro vectores vacios para llenarlos despues
  let gethistorial = []
  let getnombre = []
  let getusuarioId = []
  //itero sobre los sips para obtener el id de un usuario mediante su numero
  sipsAll.forEach((obj) => {
    if (params.numero == obj.name) {
      getusuarioId.push(obj.usuarioId)
    }
  })
  //itero sobre el usuario y pregunto si su id es el mismo encotrado arriba y obtengo su nombre
  usuarioAll.forEach((obj) => {
    if (obj.id == getusuarioId) {
